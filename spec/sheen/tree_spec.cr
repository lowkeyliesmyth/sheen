...
describe Sheen::Tree::Leaf do
  it "exposes its value and stringifies" do
    leaf = Sheen::Tree::Leaf.new("hello")
    leaf.value.should eq("hello")
    leaf.to_s.should eq("hello")
  end

  it "is shown by default but can be constructed as hidden" do
    Sheen::Tree::Leaf.new("x").hidden?.should be_false
    Sheen::Tree::Leaf.new("x", hidden: true).hidden?.should be_true
  end

  it "has no children" do
    Sheen::Tree::Leaf.new("x").children.length.should eq(0)
  end
end

describe Sheen::Tree::NodeChildren do
  it "appends and reads children in order" do
    children = Sheen::Tree::NodeChildren.new
    children.append(Sheen::Tree::Leaf.new("a")).append(Sheen::Tree::Leaf.new("b"))
  ...
  it "returns nil for out of range indices" do
    children = Sheen::Tree::NodeChildren.new.append(Sheen::Tree::Leaf.new("a"))
...
describe Sheen::Tree do
  it "sets a root value and appends string children" do
    tree = Sheen::Tree::Branch.root("root").child("a", "b")
  ...
  it "appends leaves, subtrees, and arrays to the tree" do
    sub = Sheen::Tree::Branch.root("sub").child("x")
    tree = Sheen::Tree::Branch.new.child(Sheen::Tree::Leaf.new("leaf"), sub, ["c", "d"])
  ...
  it "stringifies arbitrary values into children nodes" do
    Sheen::Tree::Branch.new.child(42).children.at(0).try(&.value).should eq("42")
  end

  it "Builds a tree root from another tree, adopting its values and children" do
    source = Sheen::Tree::Branch.root("src").child("x", "y")
    tree = Sheen::Tree::Branch.root(source)
    tree.value.should eq("src")
    tree.children.length.should eq(2)
  end

  it "auto-nests a rootless subtree onto the previous sibling leaf" do
    tree = Sheen::Tree::Branch.root("root").child("a", Sheen::Tree::Branch.new.child("b", "c"))
  ...
  it "returns an independent copy of children, leaving original untouched" do
    tree = Sheen::Tree::Branch.root("r").child("a")
    tree.children.as(Sheen::Tree::NodeChildren).append(Sheen::Tree::Leaf.new("b"))
    tree.children.length.should eq(1)
  end

  it "hides and unhides nodes" do
    tree = Sheen::Tree::Branch.new.hide
...
describe "Sheen::Tree enumerators and indenters" do
  ...
  it "rounded_enumerator rounds only the final corner" do
    kids = Sheen::Tree::NodeChildren.new.append(Sheen::Tree::Leaf.new("a")).append(Sheen::Tree::Leaf.new("b"))
...
describe "Sheen::Tree styling DSL" do
  it "carries no config until a styling setter is called" do
    Sheen::Tree::Branch.new.config.should be_nil
  end

  it "lazily builds config and returns self from a setter" do
    tree = Sheen::Tree::Branch.new
  ...
  it "applies a static enumerator style at every position" do
    tree = Sheen::Tree::Branch.root("r").child("a", "b").enumerator_style(Sheen::Style.new.bold)
  ...
  it "invokes an enumerator style block per index" do
    tree = Sheen::Tree::Branch.root("r").child("a", "b").enumerator_style do |_children, i|
  ...
  it "stores item style, root style, customer enumerator, and indenter" do
    tree = Sheen::Tree::Branch.new
      ...
      .indenter(->(_c : Sheen::Tree::Children, _i : Int32) { ">> " })
    config = tree.config.not_nil! # ameba:disable Lint/NotNil
    kids = Sheen::Tree::NodeChildren.new.append(Sheen::Tree::Leaf.new("x"))
...
describe "Sheen::Tree::Branch#render" do
  it "renders a nested tree with default brancehs and depth-3 continuations" do
    tree = Sheen::Tree::Branch.new.child("Foo", Sheen::Tree::Branch.root("Bar").child("Qux", Sheen::Tree::Branch.root("Qux-2").child("Foo", "Bar"), "Qux-3"), "Baz")
  ...
  it "prints the root value when set" do
    tree = Sheen::Tree::Branch.root("Root")
      .child("Foo", Sheen::Tree::Branch.root("Bar").child("Qux"), "Baz")
    tree.render.should eq(<<-TREE.rstrip)
      Root
      ├── Foo
      ├── Bar
  ...
  it "omits a hidden node and keeps the last-child prefix correct" do
    tree = Sheen::Tree::Branch.new.child(
      "Foo",
      Sheen::Tree::Branch.root("Bar").child(
        "Qux",
        Sheen::Tree::Branch.root("Qux-2").child("Foo-2", "Bar-2").hide,
        "Qux-3",
      ),
      "Baz",
    )
  ...
  it "renders an entirely hidden tree as an empty string" do
    tree = Sheen::Tree::Branch.new.child("Foo", "Bar").hide
  ...
  it "renders a custom enumerator and indenter" do
    tree = Sheen::Tree::Branch.new.child(
      ...
      Sheen::Tree::Branch.root("Bar").child(
        "Qux",
        Sheen::Tree::Branch.root("Qux-2").child("Foo-2", "Bar-2"),
        "Qux-3",
      ),
      "Baz",
    )
      .enumerator(->(_c : Sheen::Tree::Children, _i : Int32) { "->" })
      .indenter(->(_c : Sheen::Tree::Children, _i : Int32) { "->" })
    tree.render.should eq(<<-TREE.rstrip)
      -> Foo
      -> Bar
  ...
  it "right-aligns variable width enumerators to the widest prefix" do
    ...
    tree = Sheen::Tree::Branch.root("Ye Olde Root")
      .child("Foo", "Bar", "Baz", "Qux", "Quack")
      .enumerator(->(_c : Sheen::Tree::Children, i : Int32) { romans[i + 1] })
    tree.render.should eq(<<-TREE.rstrip)
      Ye Olde Root
        I Foo
       II Bar
  ...
  it "reconciles multiline node heights against the branch prefix" do
    tree = Sheen::Tree::Branch.root("Big\nRoot\nNode").child(
      "Foo",
      Sheen::Tree::Branch.root("Bar").child(
        "Line1\nLine2\nLine3\nLine4",
        Sheen::Tree::Branch.root("Qux").child("Foo-2", "Bar-2"),
        "Qux-2",
      ),
      "Baz\nBaz Line2"
    )
  ...
  it "uses a subtree's own enumerator config, overriding the parents" do
    tree = Sheen::Tree::Branch.root("root").child(
      "a",
      Sheen::Tree::Branch.root("sub").child("x", "y")
        .enumerator(->Sheen::Tree.rounded_enumerator(Sheen::Tree::Children, Int32)),
      "b",
    )
...
describe "Sheen::Tree.string_data" do
  it "builds a Children of leaves from strings" do
    data = Sheen::Tree.string_data("Foo", "Bar", "Baz")
  ...
  it "feeds directly into a tree as children" do
    tree = Sheen::Tree::Branch.root("Root").child(Sheen::Tree.string_data("a", "b"))
    tree.children.length.should eq(2)
    tree.children.at(1).try(&.value).should eq("b")
  end
end

describe Sheen::Tree::Filter do
  it "keeps only the nodes the predicate accepts and reindexes over them" do
    data = Sheen::Tree.string_data("Foo", "Bar", "Baz", "Nope")
  ...
  it "filters on node content and not just the index" do
    data = Sheen::Tree.string_data("Rose Quartz", "Unidentified", "Pyrite")
    filter = Sheen::Tree::Filter.new(data).filter { |node, _index| node.value != "Unidentified" }
  ...
  it "passes every node through until a predicate is set" do
    Sheen::Tree::Filter.new(Sheen::Tree.string_data("a", "b")).length.should eq(2)
  end
  ...
  it "reports zero length when the predicate rejects everything" do
    filter = Sheen::Tree::Filter.new(Sheen::Tree.string_data("a", "b", "c")).filter { |_node, _index| false }
