...
module Sheen
  # Renders a `Node` to a string. A short-lived collaborator is constructed per render from the active `TreeConfig` defined.
  struct TreePainter
    def initialize(@config : TreeConfig)
    end
    ...
    private def render_child(strs : Array(String), children : Children,
      ...
      # Descend. A child Tree with its own config overrides ours for it subtree.
      sub_painter = self
      if child.is_a?(Tree) && (child_config = child.config)
        sub_painter = TreePainter.new(child_config)
      end
